...
export default (router: Router) => {
	...
	router.post('/proposals/:id/accept-visit', async (req, res, next) => {
		const id = req.params['id'];
		const payload = req.body;
		const customerId = res.locals.entities.customer;
		...
		try {
			if (!customerId) throw new Error('Forbidden: customer only');
			return res.status(200).json(await new ProposalService(client).acceptVisit(id, customerId, payload));
		} catch (error) {
			return next(error);
		}
	});
	...
	router.patch('/proposals/:id/reopen', async (req, res, next) => {
		...
		try {
			if (!customerId) throw new Error('Forbidden: customer only');
			return res.status(200).json(await new ProposalService(client).reopen(id, customerId));
		} catch (error) {
			return next(error);
		}
	});
